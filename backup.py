...
class BackupManager:
    ...
    async def _get_backup_name_and_date(self, file_path: str) -> Tuple[str, str, bool]:
        ...
        log_message = {
            'en': f'Getting backup name and date for {file_path}: {date_pattern}',
            'ru': f'Получение имени и даты резервной копии для {file_path}: {date_pattern}',
        }
        logging.info(log_message.get(self._language, 'en'))
        ...
        if not match:
            ...
            is_original = True
            log_message = {
                'en': f'Date not found in file name: "{file_name}". New file name: "{file_name_without_date}".',
                'ru': f'Дата не найдена в имени файла: "{file_name}". Новое имя файла: "{file_name_without_date}".',
            }
            logging.info(log_message.get(self._language, 'en'))
        else:
            ...
            is_original = False
            log_message = {
                'en': f'Date found in file name "{file_name}". File name without date: "{file_name_without_date}"',
                'ru': f'Дата найдена в имени файла "{file_name}". Имя файла без даты: "{file_name_without_date}"',
            }
            logging.warning(log_message.get(self._language, 'en'))
    ...
    async def _prepare_backup_directory(self, unique_name: str, file_path: str) -> str:
        ...
        log_message = {
            ...
            'ru': f'Создаем каталог: "{backup_path}".',
        }
        logging.info(log_message.get(self._language, 'en'))
    ...
    async def _copy_file(self, file_path: str, backup_path: str) -> str:
        ...
        log_message = {
            'en': f'File: "{file_path}" copied to "{backup_file_path}".',
            'ru': f'Файл: "{file_path}" скопирован в "{backup_file_path}".',
        }
        logging.info(log_message.get(self._language, 'en'))
    ...
    async def _delete_file(self, file_path: str) -> None:
        ...
        try:
            os_remove(file_path)
            log_message = {
                'en': f'Successfully deleted: "{file_path}".',
                'ru': f'Успешно удалено: "{file_path}".'
            }
            logging.warning(log_message.get(self._language, 'en'))
        except Exception as e:
            log_message = {
                'en': f'Error deleting backup "{file_path}": {e}.',
                'ru': f'Ошибка удаления резервной копии "{file_path}": {e}.',
            }
    ...
    async def _has_sufficient_space(
        ...
        log_message = {
            'en': f'Free disk space: {free_space_gb:.2f} GB, Required size: {db_size_gb:.2f} GB, '
                  f'Minimum required space: {min_required_space_gb:.2f} GB.',
            'ru': f'Свободное место на диске: {free_space_gb:.2f} ГБ, Требуемый размер: {db_size_gb:.2f} ГБ, '
                  f'Минимально необходимое место: {min_required_space_gb:.2f} ГБ.',
        }
        logging.info(log_message.get(self._language, 'en'))
    ...
    async def _should_skip_backup(self, file_path: str) -> bool:
        ...
        if os_path.exists(hash_file_path):
            async with aio_open(hash_file_path, 'r') as hash_file:
                ...
                log_message = {
                    'en': f'Compare the current and last hashes of the file: "{file_path}". '
                          f'Current hash: {current_hash}. Last hash: {last_hash}.',
                    'ru': f'Сравниваем текущий и последний хэши файла: "{file_path}". '
                          f'Текущий хэш: {current_hash}. Последний хэш: {last_hash}.',
                }
                logging.info(log_message.get(self._language, 'en'))
                if current_hash == last_hash:
                    log_message = {
                        'en': f'No changes in file: "{file_path}", skipping backup.',
                        'ru': f'Нет изменений в файле: "{file_path}", резервное копирование пропускается.',
                    }
                    logging.info(log_message.get(self._language, 'en'))
                    return True
        
        log_message = {
            'en': f'Write the file hash: "{file_path}", to the file: "{hash_file_path}".',
            'ru': f'Записываем хэш файла: "{file_path}", в файл: "{hash_file_path}".',
        }
        logging.info(log_message.get(self._language, 'en'))
    ...
    async def _calculate_file_hash(self, file_path: str) -> tuple:
        ...
        hash_type = 'sha256'
        log_message = {
            'en': f'Calculate "{hash_type}" hash: File: {os_path.basename(file_path)}',
            'ru': f'Вычисляем хэш "{hash_type}": Файл: {os_path.basename(file_path)}',
        }
        logging.info(log_message.get(self._language, 'en'))
        hash_sha256 = sha256()
    ...
    async def _create_backup_archive(self, file_path: str) -> None:
        ...
        try:
            ...
            log_message = {
                'en': f'Creating archive: "{archive_path}" from file: "{file_path}".',
                'ru': f'Создаем архив: "{archive_path}" из файла: "{file_path}".',
            }
            logging.info(log_message.get(self._language, 'en'))
            ...
            log_message = {
                'en': f'Backup completed for "{file_path}".',
                'ru': f'Резервное копирование для "{file_path}" завершено.',
            }
            logging.info(log_message.get(self._language, 'en'))
        except Exception as e:
            log_message = {
                'en': f'Failed to backup "{file_path}": {e}.',
                'ru': f'Не удалось создать резервную копию "{file_path}": {e}.',
            }
            logging.error(log_message.get(self._language, 'en'))
    ...
    async def _is_7z_available(self) -> bool:
        ...
        try:
            ...
            log_message = {
                'en': f'Error while checking 7z availability: {e}.',
                'ru': f'Ошибка при проверке доступности 7z: {e}.',
            }
            logging.error(log_message.get(self._language, 'en'))
            return False
