...
from subprocess import check_call as sproc_check_call, CalledProcessError as sproc_CPE
import logging
...
def setup_and_run(venv_dir: str, command_list: list, executable: str = 'python') -> None:
    ...
    executable_path = get_executable_name(path_join(venv_dir, executable))

    logging.info("Устанавливаем зависимости...")
    try:
        sproc_check_call([executable_path] + command_list)
    except sproc_CPE as cpe:
...
if __name__ == "__main__":
    ...
    venv_bin_path = path_join(venv_dir_name, 'Scripts')

    try:
        if path_exists(venv_bin_path):
